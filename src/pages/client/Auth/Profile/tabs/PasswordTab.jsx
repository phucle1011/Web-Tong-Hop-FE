...
export default function PasswordTab() {
  ...
  const showPassword = (field) => {
    ...
    const toggle = (stateSetter, currentValue) => {
      const newType = input.type === "password" ? "text" : "password";
      input.type = newType;
      stateSetter(newType === "text" ? "show-password" : "hide-password");
    };

    switch (field) {
      case "old_password":
        toggle(setOldPass, oldPass);
        break;
      case "new_password":
        toggle(setNewPass, newPass);
        break;
      case "confirm_password":
        toggle(setConfirmPass, confirmPass);
        break;
      default:
        break;
    }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await axios.post(
        `${Constants.DOMAIN_API}/change-password`,
    ...
    <div className="changePasswordTab w-full">
      <div className="w-full flex xl:flex-row flex-col-reverse space-x-5 xl:items-center">
        <form
          ...
          className="w-[397px] mb-10 space-y-6"
        >
          {/* Old Password */}
          <div className="input-field">
            ...
            <div className="input-wrapper border border-[#E8E8E8] w-full h-[58px] relative">
              <input
                id="old_password"
                type="password"
                placeholder="● ● ● ● ● ●"
                {...register("old_password", { required: "Vui lòng nhập mật khẩu cũ." })}
                className="input-field placeholder:text-base text-base px-4 text-dark-gray w-full h-full bg-[#FAFAFA] focus:ring-0 focus:outline-none"
                aria-invalid={errors.old_password ? "true" : "false"}
              />
              <button
                type="button"
                className="absolute right-6 bottom-[17px] cursor-pointer"
                onClick={() => showPassword("old_password")}
                aria-label={oldPass === "show-password" ? "Ẩn mật khẩu" : "Hiện mật khẩu"}
              >
                {oldPass === "show-password" ? <EyeOpenIcon /> : <EyeOffIcon />}
              </button>
            </div>
            {errors.old_password && (
          ...
          <div className="input-field">
            ...
            <div className="input-wrapper border border-[#E8E8E8] w-full h-[58px] relative">
              <input
                ...
                type="password"
                placeholder="● ● ● ● ● ●"
                {...register("new_password", { required: "Vui lòng nhập mật khẩu mới." })}
                className="input-field placeholder:text-base text-base px-4 text-dark-gray w-full h-full bg-[#FAFAFA] focus:ring-0 focus:outline-none"
              ...
              <button
                ...
                aria-label={newPass === "show-password" ? "Ẩn mật khẩu" : "Hiện mật khẩu"}
              >
                {newPass === "show-password" ? <EyeOpenIcon /> : <EyeOffIcon />}
              </button>
            </div>
            {errors.new_password && (
          ...
          <div className="input-field">
            ...
            <div className="input-wrapper border border-[#E8E8E8] w-full h-[58px] relative">
              ...
              <button
                ...
                aria-label={confirmPass === "show-password" ? "Ẩn mật khẩu" : "Hiện mật khẩu"}
              >
                {confirmPass === "show-password" ? <EyeOpenIcon /> : <EyeOffIcon />}
              </button>
            </div>
          ...
          <div className="w-full mt-[30px] flex justify-start">
            <div className="sm:flex sm:space-x-[30px] items-center">
              <div className="w-[180px] h-[50px]">
                <button type="submit" className="yellow-btn w-full h-full">
                  <div className="text-sm font-semibold">Cập nhật mật khẩu</div>
                </button>
              </div>
              <button
                type="button"
                onClick={handleCancel}
                className="text-sm font-semibold text-qblack mt-5 sm:mt-0"
              >
                Hủy
              </button>
            </div>
...
// Biểu tượng mắt đóng (hoàn chỉnh từ code gốc)
function EyeOffIcon() {
  return (
    <svg
      width="25"
